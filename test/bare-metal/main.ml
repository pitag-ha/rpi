...
let play_sin_wave () =
  ...
  let freq = 44100. in
  let time = Float.to_int (freq *. 60.) in
  let sin_samples =
    List.init time (fun i ->
        sin (2. *. 3.1415926 *. 440. *. Float.of_int i *. (1. /. freq))
        |> ( +. ) 1.
        |> ( *. ) (0x264. /. 2.)
        |> Int.of_float)
  in
  List.iter Rpi.Pwm_jack.write sin_samples

let play_music () =
  print_endline "loading music file";
  match Music.read "/jan.wav.bin" with
  | None -> failwith "no music"
  | Some bytes -> String.iter (fun c -> Rpi.Pwm_jack.write (Char.code c)) bytes
